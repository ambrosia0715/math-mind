...
String cleanMathForDisplay(String text) {
  ...
  cleaned = cleaned.replaceAllMapped(RegExp(r'\\sqrt\s*\[([^\]]*)\]\s*\{([^}]*)\}'), (m) {
    final n = (m.group(1) ?? '').trim();
  ...
  cleaned = cleaned.replaceAllMapped(RegExp(r'([A-Za-z0-9\)\u0370-\u03FF])\^\{([^}]*)\}'), (m) {
    return '${m.group(1)}${_toSuperscript(m.group(2) ?? '')}';
  });
  cleaned = cleaned.replaceAllMapped(RegExp(r'([A-Za-z0-9\)\u0370-\u03FF])\^(-?\d{1,3})'), (m) {
    return '${m.group(1)}${_toSuperscript(m.group(2) ?? '')}';
  });

  // 6) Subscripts: a_1, a_{12}
  cleaned = cleaned.replaceAllMapped(RegExp(r'([A-Za-z\u0370-\u03FF])_\{(\d{1,4})\}'), (m) => '${m.group(1)}${_toSubscript(m.group(2) ?? '')}');
  cleaned = cleaned.replaceAllMapped(RegExp(r'([A-Za-z\u0370-\u03FF])_(\d{1,4})'), (m) => '${m.group(1)}${_toSubscript(m.group(2) ?? '')}');

  // 7) Improve multiplication display for numbers only: 2*3 -> 2×3
  cleaned = cleaned.replaceAllMapped(RegExp(r'(\d)\s*\*\s*(\d)'), (m) => '${m.group(1)}×${m.group(2)}');
